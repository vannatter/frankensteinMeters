    ...
    void tick(bool freakout, bool coma) {
        ...
        if (freakout) {
            ...
            if (beatStart_ == 0 || (int32_t)(now - nextWanderAt_) >= 0) {
                ...
                beatStart_ = now;
                nextWanderAt_ = now + (uint32_t)(8800.0f * beatVary_);
            }
            float ph = (now - beatStart_) / (8800.0f * beatVary_);
            target = 0.08f + 0.31f * (1.0f - cosf(2.0f * PI * ph)) + frand(-0.004f, 0.004f);
...
function gTarget(s,m,bm,t){
 ...
 if(m.style==='lung'&&m.mode==='follow'&&bm==='coma'){
  const ph=(t%8800)/8800;return (0.08+0.31*(1-Math.cos(2*Math.PI*ph)))*100;}
